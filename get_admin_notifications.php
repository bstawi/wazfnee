<?php
    ...
    function getDataFromRequest() {
        ...
        $GLOBALS['orderBy']     = getValueFromGet(value: 'orderBy') == null ? 'adminNotificationId DESC' : getValueFromGet(value: 'orderBy');
    }
    ...
    if($_SERVER['REQUEST_METHOD'] == 'GET') {
        ...
        if(isExistAnyDataEmpty()) {
            ...
            // Search
            $searchQuery = '(title LIKE ? OR body LIKE ?)';
            ...
            // isViewed Filter
            $isViewedValue = array_key_exists('isViewed', $filtersArray) ? $filtersArray['isViewed'] : null;
            $isViewedQuery = is_null($isViewedValue) ? "" : "AND isViewed = '$isViewedValue'";
            
            // Filters Query
            $filtersQuery = "$startDateOfCreatedQuery $endDateOfCreatedQuery $isViewedQuery";

            $total = countRows(table: 'admin_notifications', where: "$searchQuery $filtersQuery", valuesArray: array($searchValue, $searchValue));
            ...
            if(isPageRangeValid($page, $pagesNumber) || $page == 1) {
                ...
                $adminNotifications = getAll(table: 'admin_notifications', where: "$searchQuery $filtersQuery", valuesArray: array($searchValue, $searchValue), orderBy: $GLOBALS['orderBy'], limit: $limit, offset: $offset);
