<?php
    ...
    function getDataFromRequest() {
        ...
        $GLOBALS['orderBy']     = getValueFromGet(value: 'orderBy') == null ? 'socialMediaId DESC' : getValueFromGet(value: 'orderBy');
    }
    ...
    if($_SERVER['REQUEST_METHOD'] == 'GET') {
        ...
        if(isExistAnyDataEmpty()) {
            ...
            // Search
            $searchQuery = '(socialMediaId LIKE ?)';
            ...
            // isActive
            $isActiveValue = array_key_exists('isActive', $filtersArray) ? $filtersArray['isActive'] : null;
            $isActiveQuery = is_null($isActiveValue) ? "" : "AND isActive = '$isActiveValue'";
            
            // Filters Query
            $filtersQuery = "$startDateOfCreatedQuery $endDateOfCreatedQuery $isActiveQuery";

            $total = countRows(table: 'social_media', where: "$searchQuery $filtersQuery", valuesArray: array($searchValue));
            ...
            if(isPageRangeValid($page, $pagesNumber) || $page == 1) {
                ...
                $socialMedia = getAll(table: 'social_media', where: "$searchQuery $filtersQuery", valuesArray: array($searchValue), orderBy: $GLOBALS['orderBy'], limit: $limit, offset: $offset);
